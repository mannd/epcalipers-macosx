...
- Round msec and rate: Choose how msec and rate values are displayed.  Choices include round to the nearest integer (e.g. 123 msec), display 4 digits (e.g. 123.4 and 12.34), round to tenths (e.g. 123.4, 12.3), and round to hundredths (e.g. 123.45, 12.35).  Note that values in secs or values using other units are never rounded regardless of this preference and are always shown using 4 digits.
