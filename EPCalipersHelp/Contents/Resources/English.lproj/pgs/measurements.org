...
** Calibration
Calibration is necessary before any measurements can be made!  If the /Show prompts/ setting is enabled, select the *Calibrate* button or the *Caliper | Calibrate* menu item.  Follow the prompts to select a caliper and stretch it over a known interval (such as 1000 msec for time, or 10 mm for amplitude calipers).  Select the *Calibrate* button again.  In the dialog box that appears, make sure the interval matches what you are measuring.  Select one of the standard calibration intervals, or type in a custom calibration interval, entering both the calibration interval and units (e.g. /1000 msec/,  or /1 mV/).  Select the *Calibrate* button in the dialog box to set the calibration.  Note that time and amplitude calipers need to be calibrated separately.  Once calibrated, calipers will show intervals in the units used to calibrate.  Newly created calipers will use the same calibration.

If you are not using prompts, then simply select and stretch a caliper over the interval you will be using for calibration, select *Calibrate* on the toolbar, and either select from one of the standard calibration intervals, or enter a custom calibration interval.

If you use a custom time calibration interval it is important to use time units such as /msec/ or /sec/ if you want to do calculated measurements such as the corrected QT interval.  In other words, type in your custom time calibration interval as /1000 msec/ and not just /1000/.  Negative calibration intervals are treated as positive numbers (i.e. the negative sign is ignored).
